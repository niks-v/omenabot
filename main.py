...
async def mcmd(ctx: discord.ext.commands.Context, *, page:int=1):
	...
	if 1 <= page <= 4:
		mcmdembed.set_footer(name="%i/4" % page)
		mcmdembed.set_author(name="1.15.2 Full Command Documentation Page %i/4" % page)
		for cmd in cmds["%i" % page]:
			mcmdembed.add_field(name=cmd["name"], value=cmd["value"], inline=False)

	else:
		await ctx.send(f"Sorry, there's no page №{page}. Please use 1-4.")
		return
